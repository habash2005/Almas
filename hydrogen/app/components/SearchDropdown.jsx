...
export default function SearchDropdown({isOpen, onClose}) {
  ...
  // Debounced predictive search against the /search route loader
  useEffect(() => {
  ...
  const fetchedFor = fetcher.data?.q?.trim() ?? '';
  const filteredProducts =
    trimmedQuery.length > 0 && fetchedFor === trimmedQuery
      ? (fetcher.data?.results ?? []).slice(0, 5)
      : [];
