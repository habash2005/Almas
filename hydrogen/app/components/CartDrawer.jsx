...
export default function CartDrawer() {
  ...
  const cartItems = lines.map(toAlmasCartItem);
        ...
        <div className="px-6 py-4 bg-light-gray">
          {hasFreeShipping ? (
            ...
            <p className="text-[12px] text-warm-gray font-sans text-center">
              Add <span className="text-black font-medium">${freeShippingRemaining.toFixed(2)}</span> more for free shipping
            </p>
          )}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide">
          {cartItems.length === 0 ? (
            ...
            <div className="divide-y divide-black/[0.06]">
              {cartItems.map((item) => (
                <div key={item.lineId} className="flex gap-4 px-6 py-5">
                  {/* Product thumbnail */}
                  <Link
                    to={`/products/${item.handle}`}
                    onClick={() => setIsCartOpen(false)}
                    className="w-20 h-24 bg-light-gray flex items-center justify-center flex-shrink-0"
                  >
                    <img
                      src={item.image}
                      alt={item.name}
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        <Link
                          to={`/products/${item.handle}`}
                          onClick={() => setIsCartOpen(false)}
                          className="font-serif text-[15px] text-black no-underline"
                        >
                          {item.name}
                        </Link>
                      </div>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      ...
                      <div className="flex items-center border border-black/[0.12]">
                        <CartForm
                          ...
                          inputs={{
                            lines: [{id: item.lineId, quantity: item.quantity - 1}],
                          }}
                        >
                          <button
                            type="submit"
                            disabled={item.quantity <= 1 || item.isOptimistic}
                            className="w-8 h-8 flex items-center justify-center text-black hover:bg-light-gray transition-colors disabled:opacity-30"
