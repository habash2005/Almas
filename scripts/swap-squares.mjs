// Replaces ONLY the square thumbnail media (position 1) on every product with
...
for (const p of products) {
  ...
  const imgPath = resolve(ROOT, 'storefront/public/products', `${handle}-square.png`);
  ...
  if (!node) continue;
  const oldSquares = node.media.nodes.filter((m) => m.image?.url?.includes('-square'));
  ...
  if (oldSquares.length) {
    const del = await gql(
      ...
      {productId: node.id, mediaIds: oldSquares.map((m) => m.id)},
    );
  ...
  const re = await gql(
    ...
    {id: node.id, moves: [{id: newId, newPosition: '0'}]},
  );
...
console.log(`\n${swapped} squares swapped to white background.`);
